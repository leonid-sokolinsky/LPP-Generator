...
void PC_bsf_Init(bool* success) {
	...
	if (PD_n < 2) {
		cout << "Space dimension must be > 1!\n";
		*success = false;
		return;
	}

	PD_NumOfRndInequalities = PD_n;
	...
	for (int j = 0; j < PD_n; j++) {
		PD_c[j] = (PT_float_T)(PD_n - j) * PP_THETA;
		PD_centerObjectF += PD_c[j] * PP_RHO;
	}
	...
	for (int j = 0; j < PD_n; j++)
		PD_A[PD_n][j] = 1;
	PD_b[PD_n] = PP_ALPHA * (PD_n - 1) + (PT_float_T)PP_ALPHA / 2;

	for (int i = PD_n + 1; i < 2 * PD_n + 1; i++) {
		for (int j = 0; j < PD_n; j++)
			PD_A[i][j] = 0;
		PD_A[i][i - PD_n - 1] = -1;
		PD_b[i] = 0;
...
void PC_bsf_MapF(PT_bsf_mapElem_T* mapElem, PT_bsf_reduceElem_T* reduceElem, int* success) {	// For Job 0
	...
	do {
		...
		if (distToCenter > PP_RHO || distToCenter < PP_THETA) {
...
void PC_bsf_ProcessResults(		// For Job 0
	...
	if (PD_k == PD_NumOfRndInequalities + 2 * PD_n + 1) {
		*exit = true;
		return;
	}
	...
	for (int w = 0; w < BSF_sv_numOfWorkers; w++) {
		cout << "w=" << w << ")\t";
		for (int j = 0; j < PD_n; j++) cout << setw(PP_SETW) << extendedReduceElem[w].elem.a[j] << "\t";
...
void PC_bsf_ParametersOutput(PT_bsf_parameter_T parameter) {
	...
	cout << "Number of Random Inequatios: " << PD_NumOfRndInequalities << endl;
...
{
#ifdef PP_MATRIX_OUTPUT
	for (int i = 2 * PD_n + 1; i < 2 * PD_n + 1 + PD_NumOfRndInequalities; i++) {
		cout << i << ")\t";
